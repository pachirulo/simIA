...
export function buildMessages(system: SystemContext, user: string, jsonSchema: unknown): { role: string; content: unknown }[] {
...
export class OpenRouterProvider {
  ...
  async call<T>(name: CallKind, model: string, slot: Slot, system: SystemContext, user: string, schema: z.ZodType<T>, maxTokens: number, agentId: string | null = null): Promise<T | null> {
    ...
    const messages = buildMessages(system, user, jsonSchema);
