...
import { kakados, kakadosReflection, kakadosPerception } from "../test/fixtures/kakados.ts";
import { contexts } from "../test/fixtures.ts";
import { reflectionForDiagnostics, reflectionSchema, normalizeReflection } from "../src/schema/reflection.ts";
...
const rows = kakados.cases.map(row => {
  ...
  const schema = row.kind === "reflection" ? reflectionSchema(kakadosReflection(row.callId)) : null;
  return {id:row.id,schema:schema ? schema.safeParse(normalizeOptionalStrings(normalizeReflection(row.output),schema)).success : undefined,call:row.callId,attempt:row.attempt,agent:row.name,kind:row.kind,...(previous?{before:check(previous)}:{}),after:check(current)};
});
console.log(JSON.stringify({source:kakados.source,method:kakados.method,
  limits:"Offline validator/component comparison, not a new simulation or model repair success rate. Reflection uses the provider's diagnostic normalizer and the role supplements captured in the original request. Contextual schema is reconstructed from captured empty desires; generation/content returned messages, not response_format. Dialogue private aMemories/bMemories were not captured: empty in this reconstruction; speech attribution still checks the generated lines. Arrival time 360 is confirmed by original engine events.",rows},null,2));
